...
export default function Footer() {
  const boxes = [
    {
      icon: <FaFacebook />,
      link: "https://web.facebook.com/profile.php?id=61556725214046",
    },
    {
      icon: <FaInstagram />,
  ...
  return (
    <footer
      id="contact"
      className=" sofia-medium px-2.5 md:px-6 lg:px-12 bg-purple-800   relative"
    >
      <div className="border-t flex justify-between w-full items-center sofia-light text-center  border-[#808080] text-xs text-[#D0D0D0] py-6">
        <p className="text-center tracking-wider">
          © {new Date().getFullYear()}, The Prolific Woman Trybe
        </p>
        <div className="flex gap-3 md:gap-5 items-center text-white text-lg md:text-2xl cursor-pointer">
          {boxes.map((box) => (
            <Link target="_blank" href={box.link} key={box.link}>
              {box.icon}
            </Link>
          ))}
        </div>
